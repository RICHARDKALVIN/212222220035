...
class Logger {
  constructor(config = {}) {
    this.apiUrl = config.apiUrl || 'http://20.244.56.144/evaluation-service/logs';
    this.timeout = config.timeout || 5000;
    this.retries = config.retries || 3;
    this.validStacks = ['backend', 'frontend'];
    this.validLevels = ['debug', 'info', 'warn', 'error', 'fatal'];
    this.validBackendPackages = ['cache', 'controller', 'cron_job', 'domain', 'handler', 'repository', 'route'];
    this.validFrontendPackages = ['api', 'component', 'hook', 'page', 'state', 'style'];
  ...
  validateParams(stack, level, packageName, message) {
    ...
    if (!allValidPackages.includes(packageName)) {
      throw new Error(`Invalid package: ${packageName}. Must be one of: ${allValidPackages.join(', ')}`);
    }

    if (stack === 'backend' && this.validFrontendPackages.includes(packageName)) {
      throw new Error(`Package ${packageName} cannot be used with backend stack`);
    }

    if (stack === 'frontend' && this.validBackendPackages.includes(packageName)) {
      throw new Error(`Package ${packageName} cannot be used with frontend stack`);
    }
  ...
  async logWithRetry(stack, level, packageName, message, attempt = 1) {
    try {
      ...
      if (attempt < this.retries) {
        console.warn(`Log attempt ${attempt} failed, retrying... Error: ${error.message}`);
        await new Promise(resolve => setTimeout(resolve, 1000 * attempt)); // Exponential backoff
        return this.logWithRetry(stack, level, packageName, message, attempt + 1);
      } else {
        console.error(`Failed to send log after ${this.retries} attempts:`, error.message);
        throw error;
      }
    }
  }
