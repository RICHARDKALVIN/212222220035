...
const validateCreateUrl = async (req, res, next) => {
    try {
        ...
        await logger.debug('backend', 'middleware', 'Validating create URL request');
        ...
        if (validity !== undefined) {
            if (!Number.isInteger(validity) || validity <= 0) {
                await logger.warn('backend', 'middleware', `Invalid validity value: ${validity}`);
                return res.status(400).json({
        ...
        await logger.error('backend', 'middleware', `Validation error: ${error.message}`);
        res.status(500).json({
